...
class OrderController extends Controller
{
    ...
    public function showMyOrder($id)
    {
        ...
        $orderBig = Order::find($id);
        return view('client.show-my-order',compact('orders','id','orderBig'));
    }
    ...
    public function cancel(Request $request, $id)
    {
        ...
        if ($request->status == 3) {
            ...
            foreach ($orderDetails as $orderDetail) {
                ...
                Product::where('id_sp', $orderDetail->id_sp)->update(['soluongban' => $product->soluongban - $orderDetail->soluong ]);
            }
        }
        $order->tinhtrang = $request->status;
        $order->save();
        return redirect()->route('my.order')->with('success','Hủy đơn thành công.');
    }
