...
class ProductController extends Controller
{
    ...
    public function product_detail($id)
    {
        ...
        $rating = round($rating);
        $comments = Comment::where('id_sp', $id)->join('nguoidung', 'nguoidung.id_nguoidung', '=', 'binhluan.id_nguoidung')->orderBy('binhluan.created_at', 'DESC')->get(['binhluan.*', 'nguoidung.hoten']);
        $replies = Reply::all();
        ...
        return view('client.product-detail', compact('product', 'products', 'rating', 'comments', 'replies'));
    }
    ...
    {
        if ($tanggiam = intval($request->tanggiam)) {
            $oldCart = Session::has('cart') ? Session::get('cart') : null;
            $cart = new Cart($oldCart);
            if ($tanggiam == 1) $soluong = $cart->items[$id]['qty'] + 1;
            else $soluong = $cart->items[$id]['qty'] - 1;
        } else $soluong = intval($request->soluong);

        if ($soluong >= 0) {
            $oldCart = Session::has('cart') ? Session::get('cart') : null;
            $cart = new Cart($oldCart);
            $result = $cart->updateItemByOne($id, $soluong);
            Session::put('cart', $cart);
            return response()->json([
                'status' => 'success',
                "data" => $result
    ...
    {
        if ($request->id != 0) {
            $products = Product::where('id_dmsp', $request->id)->where('soluong', '>', 0)->where('tinhtrang', 1)->paginate(9);
        } else {
    ...
    public function sort(Request $request)
    {
        if ($request->sort == 0) {
            ...
            $products = Product::where('soluong', '>', 0)->where('tinhtrang', 1)->orderBy('giatien', 'DESC')->orderBy('giakhuyenmai', 'DESC')->paginate(9);
        } else {
            $products = Product::where('soluong', '>', 0)->where('tinhtrang', 1)->orderBy('giatien', 'ASC')->orderBy('giakhuyenmai', 'DESC')->paginate(9);
        }
        return response()->json([
            'status' => 200,
            'data'   => view('client.includes.product-grid', compact('products'))->render()
    ...
    {
        $products = Product::where('soluong', '>', 0)->where('tinhtrang', 1)->where('id_dmsp', $category)->paginate(9);
