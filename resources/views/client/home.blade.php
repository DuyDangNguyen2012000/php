                ...
                <div class="featured__controls">
                    <ul>
                        ...
                        @foreach ($categories as $category)
                            <li data-filter=".{{ str_replace(' ', '', $category->ten_dmsp) }}">{{ $category->ten_dmsp }}</li>
                        @endforeach
                    </ul>
                </div>
            </div>
            ...
            @foreach ($products as $product)
                <div class="col-lg-3 col-md-4 col-sm-6 mix {{ str_replace(' ', '', $product->cate_title) }}">
                    <div class="featured__item">
                        <div class="featured__item__pic set-bg" data-setbg="{{ asset($product->image->first()->url) }}">
...
<section class="latest-product spad">
    <div class="container">
        <div class="row">
            ...
            <div class="col-lg-4 col-md-6">
                <div class="latest-product__text">
                    <h4>Sản phẩm nhiều sao nhất</h4>
                    <div class="latest-product__slider owl-carousel">
                        <div class="latest-prdouct__slider__item">
                            @foreach ($product_top_rating_1 as $product)
                                <a href="{{ route('client.product.detail', ['id' => $product->id_sp]) }}" class="latest-product__item">
                                    <div class="latest-product__item__pic">
                                        <img src="{{ asset(\App\Models\Product::find($product->id_sp)->image->first()->url) }}" alt="{{ $product->ten_sp }}">
                                    </div>
                                    <div class="latest-product__item__text">
                                        <h6>{{ $product->ten_sp }}</h6>
                                        @if (strtotime(date('Y-m-d')) < strtotime($product->thoigianbatdau) || strtotime(date('Y-m-d')) > strtotime($product->thoigianketthuc))
                        ...
                        <div class="latest-prdouct__slider__item">
                            @foreach ($product_top_rating_2 as $product)
                                <a href="{{ route('client.product.detail', ['id' => $product->id_sp]) }}" class="latest-product__item">
                                    <div class="latest-product__item__pic">
                                        <img src="{{ asset(\App\Models\Product::find($product->id_sp)->image->first()->url) }}" alt="{{ $product->ten_sp }}">
                                    </div>
                        ...
                        <div class="latest-prdouct__slider__item">
                            @foreach ($product_top_rating_3 as $product)
                                <a href="{{ route('client.product.detail', ['id' => $product->id_sp]) }}" class="latest-product__item">
                                    <div class="latest-product__item__pic">
                                        <img src="{{ asset(\App\Models\Product::find($product->id_sp)->image->first()->url) }}" alt="{{ $product->ten_sp }}">
                                    </div>
