...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        if ($check) {
            return redirect()->back()->with('invalid', 'Email đã tồn tại');
        }
    ...
    {
        return view('client.auth.change_pass');
    }
    
    public function updatePass(Request $request)
    {
        if (Hash::check($request->oldpassword, Auth::user()->matkhau)) {
            if($request->input('password') === $request->input('repassword')){
                User::where('email', Auth::user()->email)->update(['matkhau' => bcrypt($request->input('password'))]);
                if(Auth::check()){
                    Auth::logout();
                }
